...
RegisterCommand("v", function(_, args)
	...
	elseif action == "buy" then
		local allowed, reason = lib.callback.await("bgarage:server:payment", false, ParkingSpotPrice, false)
		if not allowed then
		...
		for k, v in pairs(vehicles) do
			...
			if v.location == "parked" then
				getMenuOptions[#getMenuOptions + 1] = {
					...
					onSelect = function()
						local allowed, reason = lib.callback.await("bgarage:server:payment", false, GetPrice, false)
						if not allowed then
							Notify(reason, NotificationIcons[0], NotificationType[0])
								...
								options = {
									{
										...
										description = locale("menu_description_one"),
										onSelect = function()
											local allowed, reason = lib.callback.await("bgarage:server:payment", false, ImpoundPrice, false)
											if not allowed then
												Notify(reason, NotificationIcons[1], NotificationType[0])
