...
- Returns the icon for a vehicle type.

**Example:**
```lua
exports.bgarage:getVehicleIcon(model, type)
```

**Types:**
- **model**
  - `number` or `string`
    - The model or identifier of the vehicle.

- **type** _(optional)_
  - `string`
    - The type of the vehicle. If not provided, it will be determined from the vehicle class.

**Return:**
- `string`
  - The [fontawsome](https://fontawesome.com) icon corresponding to the specified vehicle type or class.

- `nil`
  - Only returns if both `model` and `type` are not provided.

#### `spawnVehicle`

- Spawns a vehicle based on the provided data and coordinates (vehicle spawning is mostly handled server-side, but this may be useful to some).

**Example:**
```lua
exports.bgarage:spawnVehicle(plate, data, coords)
```
...
- **data**
  - `Vehicle`
    - The data structure containing information about the vehicle.

- **coords**
  - `vector4`
    - The coordinates where the vehicle should be spawned.
...
- Reserves a designated garage area for storing and retrieving your owned vehicles.

**Example:**
```lua
exports.bgarage:purchaseParkingSpot(price)
```

**Types:**
- **price**
  - `number`
    - The price it costs to purchase a parking space.

#### `storeVehicle`

- Secure a vehicle within your designated garage area for future usage.

**Example:**
```lua
exports.bgarage:storeVehicle()
```

#### `vehicleList`

- Displays a comprehensive list of your owned vehicles that are in the 'parked', 'outside', and 'impound' state, accompanied by a map for tracking all 'outside' vehicles.
...
- Displays a list of your owned vehicles that are in the 'impound' state.
